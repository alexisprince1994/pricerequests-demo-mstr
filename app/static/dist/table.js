function TableManager (tableId, columns, newRowDefaults) {
  ...
  // Checking what columns are passed in.

  this.hiddenColumns = []
  this.hasColumnsHidden = false

  const allColumns = []

  this.columns.forEach(function (col) {
    allColumns.push(col.order)
  })
  ...
  */
  ...
  this.hideSystemColumns = function () {
    this.hiddenColumns = []

    const that = this

    // console.log('this.$table.columns is ' + JSON.stringify(this.$table.visible()))

    if (this.hasColumnsHidden) {
      this.hasColumnsHidden = false
      this.$table.columns(this.allColumns).visible(true)
      this.$table.columns.adjust().draw()
      return null
    } else {
      this.hasColumnsHidden = true
      this.columns.forEach(function (col) {
        if (!col.editable) {
          that.hiddenColumns.push(col.order)
        }
      })
    }

    this.$table.columns(this.hiddenColumns).visible(false)
    this.$table.draw()
    // this.$table.columns.adjust().draw()
  }
